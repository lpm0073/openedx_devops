...
- daily data backups archived into a private S3 bucket named prod-stepwisemath-mexico-mongodb-backup
...
.. code-block:: hcl
  ...
  locals {
    ...
    root_domain      = "stepwisemath.ai.ai"
...
.. code-block:: shell
  ...
  ssh bastion.web.stepwisemath.ai -i path/to/stepwisemath-mexico-live-bastion.pem
...
Use `this automated Github Actions workflow <https://github.com/StepwiseMath/openedx_devops/actions/workflows/tutor_build_image.yml>`_ to build a customized Open edX Docker container based on the latest stable version of Open edX (current maple.3) and
...
Use `this automated Github Actions workflow <https://github.com/StepwiseMath/openedx_devops/actions/workflows/tutor_deploy_prod.yml>`_ to deploy your customized Docker container to a Kubernetes Cluster.
...
- **MongoDB**. uses `AWS DocumentDB <https://aws.amazon.com/documentdb/>`_ for all MongoDB data, accessible insid the vpc as mongodb.master.web.stepwisemath.ai:27017 and mongodb.reader.web.stepwisemath.ai. Instance size settings are located in the `environment configuration file <terraform/environments/prod/env.hcl>`_, and other common configuration settings `are located here <terraform/modules/documentdb>`_. Passwords are stored in `Kubernetes Secrets <https://kubernetes.io/docs/concepts/configuration/secret/>`_ accessible from the EKS cluster.
